...
{
	sf::Mouse::setPosition(mWindow.mapCoordsToPixel(position, getMainView()));
}
...
{
	return mWindow.mapPixelToCoords(sf::Mouse::getPosition(mWindow), getMainView());
}
...
{
	return mWindow.mapPixelToCoords(sf::Touch::getPosition(touchIndex), getMainView());
}
...
void Window::setPointerPosition(const sf::Vector2f& position)
{
		...
		setMousePositionView(position, getMainView());
	#endif
}
...
void Window::screenshot()
{
	...
	if (mInfoCallback)
	{
		mInfoCallback("Screenshot : " + std::string(buffer) + ".png");
	}

	sf::Texture texture;
	texture.create(getSize().x, getSize().y);
	texture.update(mWindow);
	texture.copyToImage().saveToFile(mScreenshotPath + std::string(buffer) + ".png");
}
...
void Window::init()
{
	...
	mErrorCallback = [this](const std::string& message) { addConsoleLine("[ERROR] " + message); };
	mWarningCallback = [this](const std::string& message) { addConsoleLine("[WARNING] " + message); };
	mInfoCallback = [this](const std::string& message) { addConsoleLine("[INFO] " + message); };
	mLostFocusCallback = []() {};
